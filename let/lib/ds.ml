...
type exp_val =
  ...
  | Untuple of string list * exp_val * exp_val
...
let rec string_of_expval = function
  ...
  | RecordVal fields -> "RecordVal(" ^ string_of_record fields ^ ")"
and string_of_tree = function
  | Empty -> "Empty"
  | Node (v, l, r) ->
    "Node(" ^ string_of_expval v ^ ", " ^ string_of_tree l ^ ", " ^ string_of_tree r ^ ")"

and string_of_record fields =
  let field_to_str (name, (b, ev)) =
    name ^ "=" ^ (if b then "lazy," else "") ^ string_of_expval ev
  in
  "[" ^ String.concat "; " (List.map field_to_str fields) ^ "]"
